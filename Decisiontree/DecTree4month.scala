...
//use the features that 80-trading-day before the target date to predict the close price on the target date -- over 4-month time horizon
val lag2 = lag("Close",-80,0).over(particionwindow)
...
//cast Date column to have type date
val df15 = df14.withColumn("Date",col("Date").cast("date"))
...
// Hyperparameter: Max depth, Min isntances Per Node, Min Info Gain
...
val evaluator = new RegressionEvaluator()
  ...
  .setMetricName("rmse")
val rmse_val = evaluator.evaluate(predictions)
println("Root Mean Squared Error (RMSE) on val data = " + rmse_val)
...
// compute sMAPE value on test set
