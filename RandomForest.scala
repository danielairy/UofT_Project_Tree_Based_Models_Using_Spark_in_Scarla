/*
Author: Daniel Cho
Project Name: Prediction of Apple Stock Prices using random forrest
*/
...
import org.apache.spark.ml.feature.{Imputer,IndexToString, StringIndexer,
...
// Query data using sparkQL and create a dataframe

val InputDF1 = spark.sqlContext.sql("SELECT Date,Open,High,
                                            Low,Close,Volume
                                     FROM AAPL_CSV
...
val InputDF = InputDF1.withColumn("movingAvg5",avg(InputDF1("Close"))
                      .over(wSpec1))
                      .orderBy("id")
// Calculate the 20-day moving average
          .withColumn("movingAvg20",avg(InputDF1("Close"))
          .over(wSpec2))
          .orderBy("id")
// Calculate the 50-day moving average
          .withColumn("movingAvg50",avg(InputDF1("Close"))
          .over(wSpec3))
          .orderBy("id")
// Calculate the 100-day moving average
          .withColumn("movingAvg100",avg(InputDF1("Close"))
          .over(wSpec4))
          .orderBy("id")
// Calculate the 200-day moving average
          .withColumn("movingAvg200",avg(InputDF1("Close"))
          .over(wSpec5))
          .orderBy("id")
// Calculate the 10-day moving average
          .withColumn("movingAvg10",avg(InputDF1("Close"))
          .over(wSpec6))
          .orderBy("id")
          .toDF
// split input into training & Validation set and Test set. The data is split on "2011-05-13".
val DF1 = InputDF.filter(InputDF("date")
                 .lt(lit("2011-05-13")))
val TestDF = InputDF.filter(InputDF("date")
                    .gt(lit("2011-05-13")))
...
val Imputer1 = new Imputer()
  ...
  .setOutputCols(Array("Open_FM","High_FM","Close_FM"
                      ,"Low_FM","Volume_FM","Label_FM"))

// assemble other columns to features
val assembler = new VectorAssembler()
                .setInputCols(Array("Open_FM","High_FM","Close_FM",
                                    "Low_FM","Volume_FM","Year","Month",
                                    "Day","Day_of_Week","Quarter",
                                    "movingAvg5","movingAvg10","movingAvg20",
                                    "movingAvg50","movingAvg100","movingAvg200"))
...
val Array(trainingData, valData) = FourMonthDF
                                  .randomSplit(Array(0.75, 0.25),seed =12345)
...
val forec = predictions.select("prediction")
                        .collect()
                        .map(_(0)
                        .toString
                        .toDouble);
val actual = predictions.select("Label_FM")
                        .collect()
                        .map(_(0)
                        .toString.toDouble);
...
// Select prediction, true label and compute test error.
val evaluator2 = new RegressionEvaluator().setLabelCol("Label_FM")
                                          .setPredictionCol("prediction")
                                          .setMetricName("rmse")
val rmse2 = evaluator2.evaluate(predictions2);
println("Root Mean Squared Error (RMSE) on test data = " + rmse2)
val rfModel2 = model.stages(3)
                    .asInstanceOf[RandomForestRegressionModel];

//convert prediction and real value to array of double
val forec2 = predictions2.select("prediction")
                          .collect().map(_(0)
                          .toString.toDouble);
val actual2 = predictions2.select("Label_FM")
                          .collect().map(_(0)
                          .toString.toDouble);
// Calculate SMAPE
